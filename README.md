...
`/task-kit.plan-update` で実行可能な計画を確定した後は、参照先中心の「実行セッションパッケージ」を新しいセッションに貼り付けて `/task-kit.task-execute` を開始します。パッケージは依存関係が明確な連続ステップ群を対象にし、同一セッション内では対象範囲と一次入力に実質的な変更がない限り継続実行できます。実行前には対象ステップ群の複雑度に応じたモデル層の推奨と利用者確認を行います。
